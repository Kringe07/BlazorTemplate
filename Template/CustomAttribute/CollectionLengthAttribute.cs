...
public class CollectionLengthAttribute(int minLength, int maxLength) : ValidationAttribute
{
      ...
      protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
      {
          ...
          var count = collection.Cast<object?>().Count();
           ...
           return count > MaxLength ? new ValidationResult($"{validationContext.MemberName} must have no more than {MaxLength} items.", 
               validationContext.MemberName != null ? [validationContext.MemberName] : null) : ValidationResult.Success;
      }
}